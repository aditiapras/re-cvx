...
export const useChat = () => {
  ...
  const hasSetChatRef = React.useRef(false);

  // biome-ignore lint/correctness/useExhaustiveDependencies: Intentional - only set chat option once on mount
  React.useEffect(() => {
    if (!hasSetChatRef.current) {
      // @ts-expect-error - Type mismatch due to MessageDataPart generic variance
      editor.setOption(AIChatPlugin, "chat", chat);
      hasSetChatRef.current = true;
    }
  }, []);
